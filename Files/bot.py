...
with open("recurses/text_for_message/data_for_mess.json", "r") as data_for_mess:
    mes_data = json.load(data_for_mess)

with open(
    "recurses/text_for_message/data_for_recognize.json", "r"
) as data_for_recognize:
    recognize_data = json.load(data_for_recognize)

with open("recurses/text_for_message/callback_data.json", "r") as callback_mes_data:
...
async def start(message: types.Message):
    ...
    btn_1 = types.InlineKeyboardButton(
        text="Продолжить в текстовом формате", callback_data="text"
    )
    btn_2 = types.InlineKeyboardButton(
        text="Отправить голосовое сообщение", callback_data="voice"
    )
    # btn_3 = types.InlineKeyboardButton(text='Продолжить в мобильном приложении', callback_data='mobile')
    markup.add(btn_1, btn_2)
    markup.adjust(1, 1)
    await message.answer(text=mes_data["start_talk"], reply_markup=markup.as_markup())
...
async def choice_of_area(callback: types.CallbackQuery):
    ...
    await callback.message.answer(
        text=mes_data["choice_of_area"], reply_markup=markup.as_markup()
    )


@router.callback_query(CallbackRegionFilter())
async def route_choice(callback: types.CallbackQuery, bot: Bot):
    # запрос на обьяления по области
    request_advertisements = await get_advertisements_for_region(
        region_name=callback.data
    )
    ...
    await callback.message.answer(
        ...
        reply_markup=get_route_mk(
            count=len(request["roads"]), all_roads=request["roads"]
        ).as_markup(),
    )


@router.callback_query(CallbackRouteFilter())
async def route_for_post(callback: types.CallbackQuery, state: FSMContext, bot: Bot):
    ...
    await callback.message.answer(
        text=mes_data["choose_action"], reply_markup=markup.as_markup()
    )
...
async def choose_report(callback: types.CallbackQuery):
    ...
    for i in range(4):
        btn = types.InlineKeyboardButton(
            text=mes_data["choose_report"][i], callback_data=f"option_{i + 1}"
        )
        markup.row(btn)
    await callback.message.answer(
        text=mes_data["text_report"], reply_markup=markup.as_markup()
    )
...
async def voice_processing(message: types.Message, state: FSMContext, bot: Bot):
    ...
    if (
        skip_information[1]
        == callback_data["all_type_actions"]["report_traffic_accident"]
    ):
        await traffic_accident(message)
    elif (
        skip_information[1]
        == callback_data["all_type_actions"]["report_road_deficiencies"]
    ):
        await road_deficiencies(
            callback=None, state=state, message=message, road=skip_information[0]
        )
    elif (
        skip_information[1]
        == callback_data["all_type_actions"]["report_illegal_actions"]
    ):
        await illegal_actions(
            callback=None, state=state, message=message, road=skip_information[0]
        )
    elif (
        skip_information[1] == callback_data["all_type_actions"]["dangerous_situation"]
    ):
        ...
        skip_information[1]
        == callback_data["all_type_actions"]["recognize_gas_station"]
    ):
        await gas_station(
            callback=None, state=state, message=message, road=skip_information[0]
        )
        ...
        skip_information[1]
        == callback_data["all_type_actions"]["recognize_car_service"]
    ):
        await car_service(
            callback=None, state=state, message=message, road=skip_information[0]
        )
        ...
        skip_information[1]
        == callback_data["all_type_actions"]["recognize_parking_lot"]
    ):
        await parking_lot(
            callback=None, state=state, message=message, road=skip_information[0]
        )
        ...
        skip_information[1]
        == callback_data["all_type_actions"]["recognize_attractions"]
    ):
        await attractions(
            callback=None, state=state, message=message, road=skip_information[0]
        )
...
async def traffic_accident(message: types.Message):
    ...
    await message.answer(
        text="Для повторного выбора нажмите /start", reply_markup=return_to_start()
    )
...
async def road_deficiencies(
    ...
    if message:
        ...
        await message.answer(
            text=mes_data["locate_road_deficiencies"], reply_markup=btn_to_send_loc()
        )
    else:
    ...
    )  # вешаем статус ожидания геолокации


@router.message(F.location, ProfileStatesGroup.input_location_for_road_deficiencies)
...
async def photo_road_deficiencies(callback: types.CallbackQuery, state: FSMContext):
    ...
    await state.update_data(
        {"id_for_road_deficiencies": point_id_for_road_deficiencies}
    )

    await callback.message.answer(text=mes_data["photo_road_deficiencies"])
    await state.set_state(
        ProfileStatesGroup.input_photo_for_road_deficiencies
    )  # вешаем cтатус ожидания фото


@router.message(F.photo, ProfileStatesGroup.input_photo_for_road_deficiencies)
async def photo_road_deficiencies(message: types.Message, state: FSMContext, bot: Bot):
    # получение id точки
    data = await state.get_data()
    ...
    if status:
        await message.answer(
            mes_data["end_road_deficiencies"], reply_markup=return_to_start()
        )
        await state.clear()
...
@router.message(F.location, ProfileStatesGroup.input_location_for_illegal_actions)
...
@router.message(F.text, ProfileStatesGroup.input_description_for_illegal_actions)
...
@router.message(IsPhotoOrVideo(), ProfileStatesGroup.input_photo_for_illegal_actions)
async def input_photo_or_video(message: types.Message, state: FSMContext, bot: Bot):
    # получение id сообщения
    data = await state.get_data()
...
async def choose_que(callback: types.CallbackQuery):
    ...
    for i in range(6):
        ...
        markup.row(btn)
    await callback.message.answer(
        text=recognize_data["recognize"], reply_markup=markup.as_markup()
    )
...
async def meal(
    ...
    if message:
        ...
        # Используем объект message для отправки сообщений
        await message.answer(
            text=recognize_data["start_and_send_location"],
...
@router.message(F.location, ProfileStatesGroup.input_location_for_meal)
async def choose_meal(message: types.Message, state: FSMContext):
    ...
    list_meal = await get_request_for_dots(
        road_name=route, longitude=longitude, latitude=latitude, point_type="Cafe"
    )
    ...
    await message.answer(
        text="Для просмотра других точек нажмите /start", reply_markup=return_to_start()
    )
    await state.clear()
...
@router.message(F.location, ProfileStatesGroup.input_location_for_gas_station)
async def choose_gas_station(message: types.Message, state: FSMContext):
    ...
    list_gas_station = await get_request_for_dots(
        road_name=route, longitude=longitude, latitude=latitude, point_type="GasStation"
    )
    ...
    for i in range(count):
        ...
        distance_gas_station = round(
            list_gas_station["points"][i]["distancesFromUser"], 2
        )
    ...
    # вывод карты
    load_map(
        longitude=longitude, latitude=latitude, list_dots=list_gas_station, color="nt"
    )
    file = FSInputFile("map.png")
    await message.answer_photo(file)
    os.remove("map.png")
...
@router.message(F.location, ProfileStatesGroup.input_location_for_car_service)
async def choose_car_service(message: types.Message, state: FSMContext):
    ...
    list_car_service = await get_request_for_dots(
        road_name=route, longitude=longitude, latitude=latitude, point_type="CarService"
    )
    ...
    for i in range(count):
        ...
        distance_car_service = round(
            list_car_service["points"][i]["distancesFromUser"], 2
        )
    ...
    # вывод карты
    load_map(
        longitude=longitude, latitude=latitude, list_dots=list_car_service, color="dg"
    )
    file = FSInputFile("map.png")
    await message.answer_photo(file)
    os.remove("map.png")
...
async def parking_lot(
    ...
    await state.set_state(
        ProfileStatesGroup.input_location_for_parking_lot
    )  # вешаем статус ожидания геолокации
...
async def choose_parking_lot(message: types.Message, state: FSMContext):
    ...
    list_parking_lot = await get_request_for_dots(
        road_name=route, longitude=longitude, latitude=latitude, point_type="RestPlace"
    )
    ...
    for i in range(count):
        ...
        distance_parking_lot = round(
            list_parking_lot["points"][i]["distancesFromUser"], 2
        )
    ...
    # вывод карты
    load_map(
        longitude=longitude, latitude=latitude, list_dots=list_parking_lot, color="vv"
    )
    file = FSInputFile("map.png")
    await message.answer_photo(file)
    os.remove("map.png")
...
async def attractions(
    ...
    await state.set_state(
        ProfileStatesGroup.input_location_for_attractions
    )  # вешаем статус ожидания геолокации
...
async def choose_attractions(message: types.Message, state: FSMContext):
    ...
    for i in range(count):
        ...
        distance_attractions = round(
            list_attractions["points"][i]["distancesFromUser"], 2
        )
    ...
    # вывод карты
    load_map(
        longitude=longitude, latitude=latitude, list_dots=list_attractions, color="or"
    )
    file = FSInputFile("map.png")
    await message.answer_photo(file)
    os.remove("map.png")
...
async def dangerous_situation(message: types.Message):
    await message.answer(
        text=mes_data["dangerous_situation"], reply_markup=return_to_start()
    )
...
async def main() -> None:
    ...
    logging.basicConfig(
        ...
        handlers=[logging.FileHandler("./loging/bot.log"), logging.StreamHandler()],
    )
