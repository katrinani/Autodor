...
async def make_callback_route(region: str | None) -> list:
    ...
    if not region:
        ...
        callback = [
            request["roads"][i]["roadName"] for i in range(len(request["roads"]))
        ]
        return callback
    else:
...
def get_route_mk(all_roads: list, count: int) -> InlineKeyboardBuilder:
    """
    ...
    for i in range(count):
        temp = all_roads[i]["roadName"].split(":")[0]
        btn = types.InlineKeyboardButton(text=temp, callback_data=temp)
        markup.row(btn)
    return markup
...
def btn_to_send_loc() -> ReplyKeyboardMarkup:
    ...
    """
    kb = [
        [types.KeyboardButton(text="Отправить нынешнюю локацию", request_location=True)]
    ]
